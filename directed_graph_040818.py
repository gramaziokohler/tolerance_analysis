...
class ToleranceNetwork(Network):
    ...
    def assembly_sequence_draw(self):
        ...
        for beams in assembly_sequence:
            weight=self.assembly_sequence_network.get_vertex_attribute(int(beams), 'weight')
        ...
        for n in self.assembly_sequence:
            weight=self.assembly_sequence_network.get_vertex_attribute(int(n), 'weight')
        ...
        for tree in self.paths:
            ...
            node=str(tree[level])
            color_by_weight=self.assembly_sequence_network.get_vertex_attribute(int(node), 'weight')
        ...
        for ordered_node in self.ordering:
            for connected_node in adjacency_dictionary[ordered_node]:
                ...
                if (ordered_node,connected_node) not in added_edges and (connected_node,ordered_node) not in added_edges:
                    weight=self.assembly_sequence_network.get_vertex_attribute(int(connected_node), 'weight')
        ...
        counter=0
        for key in self.assembly_sequence:edge_beam
            #for node in list(directed_graph)[3:]:
            for node in directed_graph.body:
                key = str(key)
                if node.startswith('\t%s [' % key):
                    weight=self.assembly_sequence_network.get_vertex_attribute(int(key), 'weight')
    ...
    def build_tolerance_analysis_network(self):
        ...
        #2)Tolerance propagation through matrices concatenation
        #if attr['edge_type']=='member':
        #print u, v, attr
