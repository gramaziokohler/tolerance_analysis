...
def check_connectivity(assembly_sequence_network, current_beam, adjacency, beams_geometry, result):
    ...
    if beam_type==1:
        ...
        for nbr_node in set(nbrs): 
            ...
            #if nbr is vertical or infill
            if nbr_node_type==2 or nbr_node_type==3:
                #Count how many vertical or infill nbrs this top beam has
                vert_infills_nbrs=vert_infills_nbrs+1
                #check if any verticals or diagonal already assembled
                if str(nbr_node) in result:
                    counter=counter+1
        if counter >= vert_infills_nbrs:#it is considered to be stable if there is at least two supports
            check=True
    ...
    if beam_type==2:
        ...
        #Check if bottom beam is in general assembly         
        for beam_index in result:
            temp_type=assembly_sequence_network.get_vertex_attribute(int(beam_index), 'beam_type')
            if temp_type==0:
                counter=counter+1
        if counter>3:
            check=True
...
#self made function to locally translate frames
...
#Self made functions to map Geometry and Assembly Sequence Network
...
def get_rg_deformed_beam(deformed_beam_display_data):
    #in contrast to get_rg_no_deformed_beam: rail considers extremes and middle 
...
def get_rg_no_deformed_beam(deformed_beam_display_data):
    #in contrast to get_rg_deformed_beam: rail considers only extremes = no deformation
    rail_1=rs.AddInterpCurve([deformed_beam_display_data[0][0],deformed_beam_display_data[0][2]] ,1)
    rec_1=rs.AddPolyline(deformed_beam_display_data[1])
    rec_2=rs.AddPolyline(deformed_beam_display_data[2])
    sweep=rs.AddSweep1(rail_1,[rec_1,rec_2],True)[0]
    rs.CapPlanarHoles(sweep)
...
class ToleranceNetwork(Network):
    ...
    def propagate_tolerance_middle(self,axial_translation_x_range,axial_translation_y_range,axial_rotation_range):
        ...
        for i in range(len(self.result)):
            ...
            #If vertical beam
            elif current_beam_type==2 or current_beam_type==3:
                #Get beam data
                p_beam, ch_beam, parent_end, parent_joint, child_joint, child_end, parent_other_vertex, parent_joint_vertex, child_joint_vertex, child_other_vertex = self.get_child_and_parent_data(i)
                #Get neighbours
            ...
            #If top cord beam
                ...
                #Beam End 0
                F1=Frame(child_end.point, child_end.xaxis, child_end.yaxis)
                F1_shifted=translate_frame_own_xyz(Transformation.from_frame(F1),axial_translation_x+(axial_rotation_shift*4), axial_translation_y, 0)
                F1.transform(F1_shifted)

                #Beam End 1
                F2=Frame(child_joint.point, child_joint.xaxis, child_joint.yaxis)
                F2_shifted=translate_frame_own_xyz(Transformation.from_frame(F2),axial_translation_x+(axial_rotation_shift*4), axial_translation_y, 0)
                F2.transform(F2_shifted)
                
                #Store ends deformations
                vert_u, vert_v=get_vertices_index_GN(current_beam_index_AN) 
                self.set_edge_attribute(vert_u,vert_v,'propagated_frame_u',F2)
                self.set_edge_attribute(vert_u,vert_v,'propagated_frame_v',F1)
                ...
                #Build beam geometry
                deformed_beam=get_rg_no_deformed_beam(deformed_beam_data)
                deformed_beams.append(deformed_beam)
